							...
							0.00220636496208, 1.113207020209128e+005, 5.175186112841131e+004,
								...
								-6.209436990984312e+005, 1.444169293806241e+005, 37.50000000000000 }, {
									...
									*/
									...
									int GPS_to_GCJ(double oldx,double oldy,double& newx,double& newy)
									{
										GcjEncryptor a;
										dpoint_t oldpt(oldx,oldy);
										dpoint_t newpt(0,0);
										int res = a.encrypt(oldpt,&newpt);
									...
									int GCJ_to_GPS(double oldx,double oldy,double& newx,double& newy)
									{
										double xmid = oldx, ymid = oldy;
										double cellsize = 1;
										double realgcjx = oldx, realgcjy = oldy;
										double xmidnew,ymidnew;
										double gpsx,gpsy;
										int res = -1;
										GPS_to_GCJ(xmid,ymid,xmidnew,ymidnew);
										if (dis(xmidnew,ymidnew,realgcjx,realgcjy) <= 1e-5)
										{
											gpsx = xmid;
											gpsy = ymid;
											res = 0;
										...
										else
										{
											...
											{//?е????ı?Ҫ
												...
												GPS_to_GCJ(x1,y1,x1new,y1new);
												GPS_to_GCJ(x2,y2,x2new,y2new);
												GPS_to_GCJ(x3,y3,x3new,y3new);
												GPS_to_GCJ(x4,y4,x4new,y4new);
