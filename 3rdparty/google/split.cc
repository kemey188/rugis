...
extern uint64 SuperFastHash(const char *s, uint32 len);

HASH_TO((const char *s, uint32 slen), SuperFastHash(s, slen))
HASH_TO((const wchar_t *s, uint32 slen), SuperFastHash(reinterpret_cast<const char*>(s), sizeof(wchar_t) * slen))
